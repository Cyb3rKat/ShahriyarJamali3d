...
const SinglePortfolio = () => {
    ...
    const filteredImage = json.filter(item => item.id.toString() == id)

    return (
        <div className='w-full min-h-screen bg-white flex flex-col justify-center items-center '>

            <Image src={filteredImage[0].url} width={500} height={500} />
            <div className='mt-10 flex  space-y-5 flex-col items-center justify-center  w-[500px] '>
                <Link href={`/#portfolio`} className='bg-[#0563bb] px-6 py-2 text-white'>Back to Home page</Link>
                <div className='flex flex-col md:flex-row md:justify-between justify-center items-center w-full'>
                    <Link href={`/portfolio/${parseInt(id) - 1}`} className='text-[#0563bb]'>Previous</Link>
                    <Link href={`/portfolio/${parseInt(id) + 1}`} className='text-[#0563bb]'>Next</Link>
                </div>
            </div>
        </div>
    )
